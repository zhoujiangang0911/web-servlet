	...
	<tr>
		<td>
			<form action="system/collegeAdd.jsp" method="post" name="form" >
 			<input type="submit" value="添加新班级"  style="width: 90px;height:30px; color:olive; font-size: 15px" />
			</form> 
		</td>
		<td >
...
<table border="1" width=100%>
    ...
    <tr>
		<td colspan="7" align="right">
			...
			<a href="colSearch?action=search&type=${type}&key=${key}&pageNow=1">首页</a> &nbsp;	
			<a href="colSearch?action=search&type=${type}&key=${key}&pageNow=${data.previousPage}">上一页</a>
			<c:forEach var="i" begin="${data.startPage}" end="${data.endPage}">
			<a href="colSearch?action=search&type=${type}&key=${key}&pageNow=${i}">
			<c:choose>
			...
			<a href="colSearch?action=search&type=${type}&key=${key}&pageNow=${data.nextPage}">下一页</a> &nbsp; 
			<a href="colSearch?action=search&type=${type}&key=${key}&pageNow=${data.endPage}">末页</a>
						&nbsp;&nbsp;&nbsp;
			</label>
			<form action="colSearch" method="post"> 跳转到第
			<input type="hidden" name="action" value="search">
